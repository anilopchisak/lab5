...
// сначала рассматриваем геметрию объекта, потом освещаем (раньше рассчитывали свет сразу) 
// для каждого пикселя освещение считается один раз
// в джи буффере хранятся данные используемые для рассчета освещения
...
public:
    ...
    bool Init()
    {
        ...
        if (!m_box.LoadMesh("C:/Content/box.obj")) {
			return false;
		}

        if (!m_bsphere.LoadMesh("C:/Content/sphere.obj")) {
			return false;
		}
